    ...
    value: function request(url, method, data, header, opts) {
      ...
      if (!Formio.ignoreCache && !opts.ignoreCache && method === 'GET' && Formio.cache.hasOwnProperty(cacheKey)) {
        if (!Formio.cacheTTL.hasOwnProperty(cacheKey) || Formio.cacheTTL.hasOwnProperty(cacheKey) && Formio.cacheTTL[cacheKey] >= new Date().getTime()) {
          return Formio.cache[cacheKey].then(function (result) {
            result = (0, _cloneDeep2.default)(result);
      ...
      var responsePromise = Formio.fetch(url, options).then(function (response) {
        ...
        if (!response.ok) {
          ...
          return (_contentType && _contentType.includes('application/json') ? response.json() : response.text()).then(function (error) {
        ...
        var getResult = contentType && contentType.includes('application/json') ? response.json() : response.text();

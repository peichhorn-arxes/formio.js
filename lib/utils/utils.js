...
var _exportNames = {
  ...
  interpolate: true,
  validateInterpolationData: true,
  uniqueName: true,
  guid: true,
  getDateSetting: true,
  isValidDate: true,
...
exports.validateInterpolationData = validateInterpolationData;
...
function validateInterpolationData(string, data) {
  var invalid = false;
  var matches = /{{.+?}}/ig.exec(string) || [];
  matches.forEach(function (match) {
    var propertyPath = match.substring(2, match.length - 2).trim();
    invalid = invalid || !_lodash.default.get(data, propertyPath);
  });
  return !invalid;
}
/**

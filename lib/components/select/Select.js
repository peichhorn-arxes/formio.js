...
function (_BaseComponent) {
  ...
  function SelectComponent(component, options, data) {
    ...
    _this.currentUrl = undefined;
    _this.newUrl = _this.getUrl();
    return _this;
  }
  ...
  _createClass(SelectComponent, [{
    ...
    key: "itemSearchCriteria",
    value: function itemSearchCriteria(data) {
      if (_lodash.default.isObject(data) && this.component.customSearchCriteria) {
        return this.interpolate(this.component.customSearchCriteria, {
          item: data
        });
      }
    ...
    key: "addOption",
    value: function addOption(value, label, searchCriteria) {
      var option = {
        value: value,
        label: label,
        customProperties: {
    ...
    value: function setItems(items, fromSearch) {
      ...
      if (this.component.clientSideFilter) {
        items = items.filter(function (item) {
          return _this3.evaluate(_this3.component.clientSideFilter, {
            data: _this3.data,
            item: item
          }, 'keep');
        });
      ...
      _lodash.default.each(items, function (item) {
        _this3.addOption(_this3.itemValue(item), _this3.itemTemplate(item), _this3.itemSearchCriteria(item));
      });
      ...
      if (this.component.clearValueIfNotInItems && !items.find(function (item) {
        return _lodash.default.isEqual(_this3.dataValue, _this3.itemValue(item));
      })) {
    ...
    key: "loadItems",
    value: function loadItems(data, search, headers, options, method, body) {
      var _this4 = this;
    ...
    key: "getUrl",
    value: function getUrl(data, search) {
      var url;

      switch (this.component.dataSrc) {
        case 'resource':
          {
            url = this.options.formio ? this.options.formio.formsUrl : "".concat(_Formio.default.getProjectUrl(), "/form");
            url += "/".concat(this.component.data.resource, "/submission");
            break;
          }
    ...
    key: "updateItems",
    value: function updateItems(searchInput, forceUpdate, data) {
    ...
    value: function addInput(input, container) {
      ...
      var choicesOptions = _objectSpread({
        ...
        searchFields: this.component.searchFields || (searchField ? ["value.".concat(searchField)] : ['label', 'customProperties.searchCriteria']),
    ...
    key: "setValue",
    value: function setValue(value, flags, data) {
      ...
      var updateEventAllowed = !(flags && flags.noUpdateEvent);
      ...
      if (updateEventAllowed) {
        if (['url', 'resource'].includes(this.component.dataSrc)) {
          var newUrl = this.getUrl(data, this.dataValue);

          if (newUrl !== this.currentUrl) {
            this.newUrl = newUrl; // console.log(`${this.component.key}.setValue('${value}'): set new URL: ${this.currentUrl} -> ${this.newUrl}`);

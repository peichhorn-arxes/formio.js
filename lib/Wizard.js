  ...
  _createClass(Wizard, [{
    ...
    value: function build() {
      ...
      this.eventHandlers.forEach(function (h) {
        return _this7.removeEventListener(h.obj, h.type);
      });
      var element = this.createElement();
      var componentsContainer = this.ce('div', {
        class: 'wizard-body'
      });
      element.appendChild(componentsContainer);
      this.on('submitButton', function (options) {
        return _this7.submit(false, options);
      }, true);
      this.on('checkValidity', function (data) {
        return _this7.checkValidity(null, true, data);
      }, true);
      this.addComponents(componentsContainer, null, null);
      this.currentForm = this;
      this.on('requestUrl', function (args) {
        return _this7.submitUrl(args.url, args.headers);
      }, true);
      setTimeout(function () {
        _this7.onChange({
          noEmit: true
    ...
    value: function buildWizardHeader() {
      ...
      this.pages.forEach(function (page, i) {
        ...
        var clickable = i <= _this8.page + 1 && clickableFlag;
        ...
        if (clickable) {
          _this8.addEventListener(pageButton, 'click', function (event) {
            ...
            if (i <= _this8.page || _this8.checkValidity(_this8.submission.data, true)) {
              _this8.setPage(i);
            }
          });
        }
